...
import {
  ...
  User, PartyPopper, Utensils, MoreHorizontal
} from "lucide-react";
...
// --- 💰 MOTORE PREZZI (Regole esatte da prompt) ---
...
// 🔧 COMPONENTE ESTRATTO: Opzioni Personalizzazione Bevande (Riusabile)
...
// 🔧 Selettore Bevande (Modalità Singola)
...
// 🔧 Selettore Dolci (Modalità Singola)
...
type ActionState = {
  ...
  orderId?: string; // 🔧 Diciamo a TypeScript che orderId PUÒ esistere (opzionale)
};
...
export default function PrenotaColazionePage() {
  ...
  // Stato Customizzazione Bevande (Modalità 5+)
...
// 💰 CALCOLO CARRELLO & PREZZI
  ...
  // 🧾 COMPONENTE SCONTRINO (Styled come Ticket)
        ...
        <div className="space-y-3 mb-6 max-h-[350px] overflow-y-auto no-scrollbar pr-2">
            {cartData.items.length === 0 ? (
                ...
                cartData.items.map((item, idx) => (
                    <div key={idx} className="flex justify-between items-start gap-2 text-amber-950">
              ...
              <form action={formAction} className="w-full space-y-6">
                ...
                <section>
                  ...
                  <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 space-y-6 w-full">
                                            ...
                                            <div key={d.id} className="border border-slate-100 rounded-lg overflow-hidden">
                                                <div className="flex justify-between items-center p-2 bg-white hover:bg-slate-50">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {/* 🔧 Tasto per aprire personalizzazione Bevanda */}
                                                ...
                                                {/* ↳ Sub-righe per bevande personalizzate */}
                                                {subKeys.map(sk => (
                                                ...
                                                {/* 🔧 Pannello Personalizzazione Condiviso */}
                           ...
                           <div>
                               ...
                               <div className="space-y-2 max-h-[350px] overflow-y-auto pr-1">
                                   ...
                                   {PASTRIES_DATA.filter(p => p.id !== 'nessuno' && !p.label.toLowerCase().includes('nessun') && !p.label.toLowerCase().includes('grazie')).map(p => {
                                       ...
                                       // ↳ Sotto-righe per varianti dietetiche non attualmente selezionate nel filtro
                                       ...
                                       return (
                                           <div key={p.id} className="border border-slate-100 rounded-lg overflow-hidden">
                                                <div className={clsx("flex justify-between items-center p-2 bg-white", isDisabled ? "opacity-40 grayscale" : "hover:bg-slate-50 transition-colors")}>
                                                    ...
                                                    <div className="flex items-center gap-2 bg-slate-50 p-1 rounded-md border border-slate-200">
                                                        {/* Il - è abilitato se la currentQty > 0, il + è disabilitato se il prodotto non è compatibile col filtro */}
                                                ...
                                                {/* Mostra le varianti in carrello che non corrispondono al filtro attivo */}
                                                {subKeys.map(sk => (
                                                    <div key={sk} className="flex justify-between items-center p-2 pl-4 bg-slate-50/80 border-t border-slate-100">
                    ...
                    {/* ⚠️ Warning Globale Allergeni Nutella+GlutenFree */}
                    ...
                    {peopleCount !== '5+' && menus.map((m, idx) => {
                        if (idx >= (peopleCount as number)) return null;
                        return (
                            <Fragment key={idx}>
                                <input type="hidden" name={`drink${idx + 1}`} value={m.drink} />
                                <input type="hidden" name={`croissant${idx + 1}`} value={m.pastry} />
                            </Fragment>
                        )
                    })}
                  </div>
                ...
                <section>
                  ...
                  <div className="bg-white p-3 rounded-2xl shadow-sm border border-slate-200 space-y-3 w-full">
                    ...
                    <input type="hidden" name="spremuteCount" value={spremuteCount} />
                ...
                <section>
                    <h3 className="text-sm font-bold text-slate-900 mb-3 flex items-center gap-2">
                        <span className="w-5 h-5 rounded-full bg-amber-900 text-white text-[10px] flex items-center justify-center font-bold">4</span>
                        Note per lo Staff
                    </h3>
                ...
                <section>
                  ...
                  <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200 space-y-4 w-full">
                    ...
                    <div className="space-y-3 w-full">
                      ...
                      {delivery === 'domicilio' ? (
                        <div className="space-y-2">
                          <input ref={addressInputRef} type="text" name="address" autoComplete="off" placeholder="Indirizzo (Inizia a digitare...)" className={clsx("w-full px-4 py-3 rounded-xl border bg-amber-50/50 text-sm focus:bg-white outline-none transition-colors animate-fade-in", addressError ? "border-red-500 focus:border-red-500 text-red-600" : "border-amber-200 focus:border-amber-400")} required onChange={(e) => setAddress(e.target.value)} />
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
                    <div className="flex flex-col gap-2">
                        <button type="button" onClick={() => setPaymentMethod("instore")} className={clsx("relative p-4 rounded-2xl border-2 text-left transition-all duration-200 flex flex-col gap-2 h-full", paymentMethod === "instore" ? "border-amber-900 bg-amber-50 shadow-md" : "border-slate-100 bg-white text-slate-400 hover:border-amber-200")}>
                          ...
                          {paymentMethod === "instore" && <div className="absolute top-3 right-3 w-4 h-4 bg-amber-900 rounded-full flex items-center justify-center"><div className="w-1.5 h-1.5 bg-white rounded-full" /></div>}
                        </button>
                        {paymentMethod === "instore" && (
                             <div className="bg-slate-100 border border-slate-200 rounded-xl p-3 text-[10px] text-slate-600 animate-fade-in"><p className="font-bold flex items-center gap-1.5 mb-1 text-slate-800"><Info size={12}/> Info Pagamento</p>Selezionando questo metodo pagherai direttamente al nostro staff al momento del ritiro in sede o all'arrivo del nostro fattorino al tuo domicilio.</div>
                        )}
                    </div>
                    <div className="flex flex-col gap-2">
                        <button type="button" onClick={() => setPaymentMethod("card")} className={clsx("relative p-4 rounded-2xl border-2 text-left transition-all duration-200 flex flex-col gap-2 h-full", paymentMethod === "card" ? "border-indigo-600 bg-indigo-50/50 shadow-md" : "border-slate-100 bg-white text-slate-400 hover:border-indigo-200")}>
                          ...
                          {paymentMethod === "card" && <div className="absolute top-3 right-3 w-4 h-4 bg-indigo-600 rounded-full flex items-center justify-center"><div className="w-1.5 h-1.5 bg-white rounded-full" /></div>}
                        </button>
                        {paymentMethod === "card" && (
                             <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3 text-[10px] text-indigo-800 animate-fade-in"><p className="font-bold flex items-center gap-1.5 mb-1 text-indigo-900"><ShieldCheck size={12}/> Sicurezza Stripe</p>Verrai reindirizzato sul portale sicuro Stripe per il pagamento. Accettiamo <strong>Visa, Mastercard, Amex, Apple Pay e Google Pay.</strong></div>
                        )}
                    </div>
                  </div>
                ...
                <div className="sticky bottom-3 z-40 px-1 pb-[env(safe-area-inset-bottom)]">
                  <div className="backdrop-blur-xl bg-white/90 p-2 rounded-2xl border border-white shadow-2xl">
                    <SubmitButton label={paymentMethod === 'card' ? "Vai al Pagamento" : "Conferma Ordine"} price={cartData.total} />
                    {state.message && !state.success && <div className="bg-red-50 text-red-600 p-2 rounded-xl text-center text-[11px] mt-2 border border-red-100 font-bold">{state.message}</div>}
                  </div>
                </div>

                {/* 🔧 INPUT NASCOSTI PER IL SERVER */}
                <input type="hidden" name="totalPrice" value={cartData.total || 0} />
                {/* Passiamo l'intero scontrino convertito in stringa JSON! */}
                <input type="hidden" name="cartDetails" value={JSON.stringify(cartData.items)} />
              </form>
            )}
          </div>

          {/* LATO DESTRO (DESKTOP SCONTRINO - 🔧 Sticky Perfezionato) */}
