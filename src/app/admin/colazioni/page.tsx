...
// 🔧 1. NUOVO TIPO ORDINATO: Niente più drink_1, peluche, ecc. Solo campi essenziali.
type Order = {
  id: string;
  created_at: string;
  full_name: string;
  phone: string;
  address?: string;
  box_type: string; // Es: "Colazione per 1", "Colazione per 5+"
  quantity: number;
  total_price: number;
  status: string;
...
function getVisualStatus(dbStatus: string): 'pending' | 'confirmed' | 'completed' | 'cancelled' {
  switch (dbStatus) {
    ...
    case 'bozza_in_attesa': // Opzionale se li mostri
    case 'pending':
      return 'pending';
    case 'confirmed':
      return 'confirmed';
...
export default function AdminDashboard() {
  ...
  const fetchOrders = async () => {
    ...
    const { data, error } = await supabase
      .from('web_orders')
      .select('*')
      .neq('status', 'bozza_in_attesa') // Nascondiamo ordini abbandonati su stripe
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans pb-20 relative">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-amber-700 text-sm font-bold mt-1">
                    Gestione Prenotazioni Colazione ☕
                  </p>
                </div>
              </div>

              <div className="grid grid-cols-2 sm:flex gap-2">
                <button onClick={handleLogout} className="flex justify-center items-center gap-2 px-4 py-3 bg-white text-slate-700 border border-slate-200 rounded-xl hover:bg-red-50 hover:text-red-600 hover:border-red-200 font-bold shadow-sm text-sm transition-colors">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1 flex gap-2">
                <div className="relative flex-1">
                  ...
                  />
                  {searchQuery && (
                    <button onClick={() => setSearchQuery("")} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-400 hover:text-red-500"><X size={18} /></button>
                  )}
                </div>
                          ...
                          {order.address && order.address !== "RITIRO IN SEDE" && (
                            <div className="flex justify-between items-center gap-2">
                              ...
                              {order.delivery_type === 'domicilio' && (
                                <a href={mapsUrl} target="_blank" rel="noopener noreferrer" className="ml-auto bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white border border-blue-200 p-1 rounded transition-all shrink-0" title="Apri Mappa"><Navigation size={12} /></a>
                              )}
                            </div>
                          )}
                        </div>
      ...
      {/* --- 🔧 SEZIONE STAMPA AGGIORNATA --- */}
      <div className="hidden print:block p-8 bg-white text-black">
        <div className="flex justify-between items-end mb-8 border-b-2 border-black pb-4">
          <div>
            <h1 className="text-3xl font-bold uppercase tracking-widest">Lista Produzione Colazioni</h1>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b-2 border-black text-xs uppercase">
              ...
              <th className="py-2">Ordine & Dettagli</th>
              <th className="py-2 w-1/4 text-right">Logistica / Note</th>
            </tr>
          </thead>
                ...
                return (
                  <tr key={order.id} className="border-b border-gray-300">
                    ...
                    <td className="py-3 align-top pr-4">
                      ...
                      <div className="text-xs mt-1 text-gray-600">{order.phone}</div>
                    </td>
                    <td className="py-3 align-top">
                      <div className="font-bold mb-1 uppercase text-xs border border-black inline-block px-1 rounded">{order.box_type || "Colazione"}</div>
                      <ul className="text-sm leading-snug space-y-1 mt-2 list-disc pl-4">
                        {scontrino.map((item, i) => <li key={i} className="font-medium">{item}</li>)}
                      </ul>
                    </td>
